...
async function add_messages(msg, scroll) {
  ...
  if (typeof msg.name !== "undefined") {
    ...
    var content =
      ...
      '</b><p>' +
      msg.message +
      '</p>'+`<button class="btnTrans" onclick=Translate(event)>翻译</button>` +'<span class="res_translate"></span>'+'<span class="time-right">' +
        n +
        '</span></div>';
    ...
    if (username == msg.name) {
      content =
        ...
        "</b><p>" +
        msg.message +
        '</p>'+`<button class="btnTrans" onclick=Translate(event)>翻译</button>`+'<span class="res_translate"></span>' +'<span class="time-left">' +
        n +
        '</span></div>';
    }
    // update div
